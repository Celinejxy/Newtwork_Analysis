...
net <- graph_from_edgelist(as.matrix(adviceE[,1:2]),directed=TRUE)
...
eAttr <- attributes[attributes$ID %in% V(net)$name,]
eAttr <- eAttr[match(V(net)$name, eAttr$ID),]
eAttr
V(net)$Department <- eAttr$Department
colours <- c("#e00f0f", "#b5af36", "#9440dd", '#2d2d48', '#923f4d', '#136d13', '#18a095')
V(net)$depColors<- as.character(factor((eAttr$Department),levels = sort(unique(eAttr$Department)),labels = colours))
##graph the true class as the department 
plot(simplify(net), vertex.shape ='none', vertex.label.color= V(net)$depColors,
    vertex.label.cex= 1, edge.arrow.size=0.4, layout= layout_save)
V(net)

##Vignett 1. Brokerage roles&position
bNet <- as.network(adviceE, loops =TRUE, multiple=TRUE, directed=TRUE)
bNet %v% 'department' <- as.character(eAttr$Department)
adviceBro<-  brokerage(bNet, 'department')
head(adviceBro$raw.nli)


##Vignett 2 Structural Equivalence 
...
plot(simplify(net), vertex.color = group_ids_5$cluster, 
     layout = layout_save)
plot(simplify(net), vertex.color = group_ids_4$cluster, 
     layout = layout_save)
