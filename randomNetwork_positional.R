...
#name&position geenrator 
prob<- sapply(runif(70,0,1)*0.1,rep, 70) #adjacency matrix 
random <- rgraph(70,tprob = prob, ,mode = 'digraph', return.as.edgelist = TRUE)
random[,1]<-  paste('a', random[,1], sep='')
random[,2]<-  paste('a', random[,2], sep='')
head(random)

##Structural Equivalence
eq<- equiv.clust(random)
class(eq)
blockM<- blockmodel(random, eq, h=10)
plot(blockM)



netRandom <- graph_from_edgelist(as.matrix(random[,1:2]),directed=TRUE)
layout_save <- layout_with_fr(netRandom)

##Loading Attributes 
attributes <- read.csv('Adjacency_ds/attr.csv',header = T)
head(attributes)
attributes$Department <- as.factor(attributes$Department)
eAttr <- attributes[attributes$ID %in% V(netRandom)$name,]
eAttr <- eAttr[match(V(netRandom)$name, eAttr$ID),]
eAttr
V(netRandom)$Department <- eAttr$Department
colours <- c("#e00f0f", "#b5af36", "#9440dd", '#2d2d48', '#923f4d', '#136d13', '#18a095')
V(netRandom)$depColors<- as.character(factor((eAttr$Department),levels = sort(unique(eAttr$Department)),labels = colours))
##graph the true class as the department 
plot(simplify(netRandom), vertex.shape ='none', vertex.label.color= V(netRandom)$depColors,
    vertex.label.cex= 1, edge.arrow.size=0.4, layout= layout_save)

##Vignett 1. Random Network- Brokerage roles&position
...
plot(simplify(netRandom), vertex.color = group_ids_5$cluster, 
     layout = layout_save)
plot(simplify(netRandom), vertex.color = group_ids_4$cluster, 
     layout = layout_save)

#Vignett 3. Block Modeling with CONCOR
net_cor = cor(adj_mat)
...
summary(all_output)
plot(simplify(netRandom), vertex.color = clusters)

# Vignett 3.Isomorphoc Local Graph
local_graphs <- make_ego_graph(netRandom, order = 3)
...
# Vignett 4. Stochastic Block Models
library(blockmodels)
...
plot(sbm_output)
plot(simplify(netRandom), vertex.color = class_assignments)


# Vignett 5. Feature-Based 
...
local_man <- function(netRandom, vertex_name, steps = 2){
...
onestep_triad_counts <- lapply(V(netRandom)$name, FUN = function(x) local_man(net, x, steps = 1))
twostep_triad_counts <- lapply(V(netRandom)$name, FUN = function(x) local_man(net, x, steps = 2))
threestep_triad_counts <- lapply(V(netRandom)$name, FUN = function(x) local_man(net, x, steps = 3))
...
indegrees <- igraph::degree(netRandom, mode = "in")
outdegrees <- igraph::degree(netRandom, mode = "out")
...
# plot the results
plot(netRandom, vertex.color = cluster_ids)
graph_features
